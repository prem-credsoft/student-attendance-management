            ...
            <div class="card-body">
              <div class="table-responsive">
                <table id="example2" class="table table-bordered table-striped">
                  ...
                  <tbody>
                    <?php
                    $rows = selectFromTable('studentinfo', ['id', 'name', 'batch', 'batch_name', 'pending_fees', 'mobile_number', 'dob', 'gender', 'aadharcard_number', 'profession', 'address', 'reference', 'joining_purpose', 'extratime_daily', 'gmail_id', 'father_name', 'father_number', 'father_profession', 'workplace_address', 'mother_name', 'home_number', 'admission_time', 'total_fees', 'paid_fees', 'pending_fees', 'discount'], ['alumnistudent' => 0]);
                    foreach ($rows as $row) {
                      echo "<tr>";
                      echo "<td>" . $row['id'] . "</td>";
...
<script>
  ...
  function confirmMoveToAlumni(id) {
  var confirmAction = confirm("Are you sure you want to move this student to alumni?");
  if (confirmAction) {
    $.post('move_to_alumni.php', { student_id: id }, function(response) {
